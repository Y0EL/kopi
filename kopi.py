...
st.title("☕🍵 Kopigoyang slebew ama Teh, WOEE NGE TEHH!!")
...
with st.expander("Pengaturan Kafe", expanded=False):
    use_random_cafe = st.checkbox("Pake Nama Kafe Random", value=True)
    if use_random_cafe:
        cafe_name = random.choice(CAFE_NAMES)
    else:
        cafe_name = st.selectbox("Pilih Nama Kafe", CAFE_NAMES)

    st.session_state['order_number'] = st.text_input("Nomor Pesanan", value=st.session_state['order_number'])

    use_random_cashier = st.checkbox("Pake Kasir Random", value=True)
    if use_random_cashier:
        cashier_name = random.choice(CASHIERS)
    else:
        cashier_name = st.selectbox("Pilih Kasir", CASHIERS)

with st.expander("Pengaturan Waktu", expanded=False):
    use_current_datetime = st.checkbox("Pake Tanggal dan Waktu Sekarang", value=False)
    if use_current_datetime:
        receipt_datetime = datetime.now()
    else:
        col1, col2, col3 = st.columns(3)
        with col1:
            receipt_date = st.date_input("Pilih Tanggal", value=datetime.now())
        with col2:
            receipt_hour = st.number_input("Jam", min_value=0, max_value=23, value=datetime.now().hour)
        with col3:
            receipt_minute = st.number_input("Menit", min_value=0, max_value=59, value=datetime.now().minute)
        receipt_datetime = datetime.combine(receipt_date, datetime.min.time()).replace(hour=receipt_hour, minute=receipt_minute)

with st.expander("Pengaturan Pesanan", expanded=True):
    num_items = st.number_input("Jumlah item", min_value=1, max_value=len(COFFEE_ITEMS + TEA_ITEMS + SNACK_ITEMS), value=1)
    use_random_items = st.checkbox("Bikin Item Random", value=True)
    ...
    if use_random_items:
        ...
        for i in range(num_items):
            ...
            with col1:
                item = st.selectbox(f"Item {i+1}", available_items, key=f"item_{i}")
            with col2:
                quantity = st.number_input(f"Jumlah {i+1}", min_value=1, max_value=10, value=1, key=f"qty_{i}")
            with col3:
                price = st.number_input(f"Harga {i+1}", min_value=1000, value=15000, step=1000, key=f"price_{i}")
...
tip = st.number_input("Tip", min_value=0, value=0, step=1000)
total = subtotal + tax + tip

st.write(f"Subtotal: Rp {subtotal:,.0f}")
st.write(f"Pajak (12%): Rp {tax:,.0f}")
st.write(f"Tip: Rp {tip:,.0f}")
st.write(f"Total: Rp {total:,.0f}")

with st.expander("Pengaturan Pembayaran dan Pesan", expanded=False):
    use_random_payment = st.checkbox("Pake Metode Pembayaran Random", value=True)
    if use_random_payment:
        payment_method = random.choice(PAYMENT_METHODS)
    else:
        payment_method = st.selectbox("Pilih Metode Pembayaran", PAYMENT_METHODS)

    use_random_message = st.checkbox("Pake Pesan Motivasi Random", value=True)
    if use_random_message:
        motivational_message = random.choice(MOTIVATIONAL_MESSAGES)
    else:
        motivational_message = st.selectbox("Pilih Pesan Motivasi", MOTIVATIONAL_MESSAGES)

logo_options = ["1.png", "2.png", "3.png"]
selected_logo = random.choice(logo_options)

if st.button("Bikin Struk"):
...
print("Kode udah diupdate sesuai permintaan lu, bro!")
